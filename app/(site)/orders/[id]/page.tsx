...
export default async function OrderDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="container max-w-6xl py-12 px-4 md:px-8">
      <div className="mb-8">
        <Link href="/orders" className="text-sm text-slate-600 hover:text-slate-900 mb-2 inline-block">
          ← Back to Orders
        </Link>
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-4xl font-bold text-slate-900 mb-2">Order #{order.id.slice(0, 8).toUpperCase()}</h1>
          ...
          <Badge variant="outline" className={`${getStatusColor(order.status)} text-base px-4 py-2`}>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-1 text-sm">
                  {order.customer_name && <p className="font-medium text-slate-900">{order.customer_name}</p>}
                  {order.customer_email && <p className="text-slate-600">{order.customer_email}</p>}
                  {order.shipping_address && <p className="text-slate-600">{order.shipping_address}</p>}
                  ...
                  {order.shipping_country && <p className="text-slate-600">{order.shipping_country}</p>}
                </div>
              </CardContent>
            </Card>
          )}
          ...
          <Card>
            ...
            <CardContent>
              {statusHistory && statusHistory.length > 0 ? (
                  ...
                  {statusHistory.map((history: any, index: number) => (
                    <div key={history.id} className="flex gap-3">
                      ...
                      <div className="flex-1 pb-4">
                        ...
                        {history.note && <p className="text-sm text-slate-600 mt-1">{history.note}</p>}
                ...
                <p className="text-sm text-slate-600">No status updates available</p>
              )}
            </CardContent>
          </Card>
