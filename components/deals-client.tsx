...
export function DealsClient({
  ...
  const [sortBy, setSortBy] = useState(searchParams.get("sort") || "discount-desc")
  ...
  }, [searchQuery, selectedCollection, minDiscount, sortBy, currentPage, router])
  ...
  const filteredProducts = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case "price-asc":
          return a.price * (1 - a.discount_percentage / 100) - b.price * (1 - b.discount_percentage / 100)
        case "price-desc":
          return b.price * (1 - b.discount_percentage / 100) - a.price * (1 - a.discount_percentage / 100)
        case "name":
  ...
  const activeFiltersCount = (searchQuery ? 1 : 0) + (selectedCollection !== "all" ? 1 : 0) + (minDiscount > 0 ? 1 : 0)
  ...
  const FilterContent = () => (
    <div className="space-y-6">
      ...
      <div>
        <Label className="text-sm font-medium mb-3 block">Minimum Discount: {minDiscount}%</Label>
        <Slider
          value={[minDiscount]}
          onValueChange={(value) => {
  ...
  return (
    <div className="container py-8 px-4 md:px-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground text-pretty">Save big on your favorite fashion items</p>
      </div>
