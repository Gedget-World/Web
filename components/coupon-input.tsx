  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      if (subtotal < coupon.min_purchase_amount) {
        setError(`Minimum purchase of $${coupon.min_purchase_amount.toFixed(2)} required`)
        setLoading(false)
        return
      }
  ...
  if (appliedCoupon) {
    return (
      <div className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg">
        ...
        <Button variant="ghost" size="icon" onClick={handleRemoveCoupon} className="h-8 w-8 text-green-700">
          <X className="h-4 w-4" />
        </Button>
      </div>
    )
